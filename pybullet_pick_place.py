...
tg_pick_line=[]
for i in range(1,13):
    tg_pick_line.append([traj[-1][0],traj[-1][1],traj[-1][2] - 0.02*i])
...
tg_pick_l=[]
for i in range(1,15):
    tg_pick_l.append([tg_pick_line[-1][0],tg_pick_line[-1][1],tg_pick_line[-1][2] + 0.02*i])
for i in range(len(tg_pick_l)):
    pos = pb.calculateInverseKinematics(robot,8,tg_pick_l[i],[np.cos(np.pi/4),-np.sin(np.pi/4),0,0])
